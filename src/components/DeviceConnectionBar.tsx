...
export const DeviceConnectionBar: React.FC<DeviceConnectionBarProps> = ({
  ...
  const [isKillingConnections, setIsKillingConnections] = React.useState(false);
  const [killSuccess, setKillSuccess] = React.useState(false);
  const [screenCmdCopied, setScreenCmdCopied] = React.useState(false);
  const [showTroubleshootModal, setShowTroubleshootModal] = React.useState(false);
  ...
  return (
    <div>
        ...
        <div className="bg-rose-950/95 border-b border-rose-800/80 px-3 py-2.5 text-xs text-rose-200 flex flex-wrap items-center justify-between gap-3 shadow-md">
          ...
          <div className="flex items-center gap-2 shrink-0 flex-wrap">
            ...
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-md bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium border border-slate-700 transition"
              title="Switch to virtual simulator profile to test in preview mode"
            >
            ...
            <button
              ...
              className="text-rose-400 hover:text-rose-200 p-1.5 rounded hover:bg-rose-900/50 transition"
              title="Dismiss error"
            >
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="flex items-center gap-1.5 bg-slate-950 p-1 rounded-md border border-slate-800">
          <button
            ...
            title={
              isIframe
                ? 'WebSerial requires opening in a dedicated tab (browser security blocks WebSerial in iframes)'
                : hasWebSerial
                ? 'Physical USB/UART WebSerial Port'
                : 'WebSerial not available in this browser'
            }
          >
            <Cable className="w-3.5 h-3.5" />
            <span>WebSerial API</span>
            {isIframe ? (
              <span className="text-[10px] px-1 py-0.2 rounded bg-amber-950/80 border border-amber-800/60 text-amber-300 font-mono">
                Tab Req
              </span>
            ) : hasWebSerial ? (
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        ) : !isConnected && isWebSerialIframeRestricted ? (
          <div className="flex items-center gap-1.5">
            <a
              href={typeof window !== 'undefined' ? window.location.href : '#'}
              target="_blank"
              rel="noreferrer"
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-md font-medium text-xs bg-cyan-600 hover:bg-cyan-500 text-white border border-cyan-500 shadow-sm transition"
              title="Open the app in a dedicated browser tab to access your physical USB serial hardware"
            >
              <ExternalLink className="w-3.5 h-3.5" />
              <span>Open in New Tab to Connect</span>
            </a>
            <button
              type="button"
              onClick={() => {
              ...
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-md text-xs font-medium text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 border border-slate-700 transition"
              title="Run embedded simulator in preview mode without opening a new tab"
            >
